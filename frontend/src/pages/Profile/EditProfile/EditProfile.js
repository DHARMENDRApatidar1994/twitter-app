...
function EditChild ({dob,setDob}){
    ...
    return(
        <React.Fragment>
            ...
            >
                <Box sx={{ ...style,width: 300, height: 370}}>
                    <div className='text'>
    ...
    const HandleSave = async () => {
        const editedInfo = {
          ...
          if(editedInfo){
            ...
            await axios.patch(`https://twitter-app-api.vercel.app/userUpdates/${user?.email}`,editedInfo)  }     
            ...
            >
                <Box sx={style} className='modal'>
                    ...
                    <form className='fill-content'>
                        <TextField className='text-field' fulWidth label='Name' id='fullWidth' variant='filled' onChange={(e) => setName
                            (e.target.value)} defaultValue={loggedInUser[0]?.name ? loggedInUser[0].name : ''} />
                        <TextField className='text-field' fulWidth label='Bio' id='fullWidth' variant='filled' onChange={(e) => setBio
                            (e.target.value)} defaultValue={loggedInUser[0]?.bio ? loggedInUser[0].bio : ''} />
                        <TextField className='text-field' fulWidth label='Location' id='fullWidth' variant='filled' onChange={(e) => setLocation
                            (e.target.value)} defaultValue={loggedInUser[0]?.location ? loggedInUser[0].location : ''} />
                        <TextField className='text-field' fulWidth label='Website' id='fullWidth' variant='filled' onChange={(e) => setWebsite
